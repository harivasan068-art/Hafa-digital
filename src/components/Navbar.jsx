...
import { Shield, LogOut, Settings, Server, CheckCircle2, User, ChevronDown } from 'lucide-react';
...
export const Navbar = ({ onOpenSettings }) => {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 w-full bg-white/90 dark:bg-zinc-900/90 backdrop-blur-md border-b border-slate-200 dark:border-zinc-800 px-4 sm:px-6 py-3.5 flex items-center justify-between transition-colors duration-300">
        {/* Brand Logo & Title */}
        <div className="flex items-center space-x-3">
          <img src="/logo.png" alt="HafA DIGITAL" className="w-9 h-9 object-contain rounded-full shadow-sm" />
          <div>
            <div className="flex items-center space-x-2">
              <h1 className="font-black text-base sm:text-lg text-slate-900 dark:text-white tracking-tight">HafA DIGITAL</h1>
              <span className="text-[10px] px-2 py-0.5 rounded-full font-extrabold uppercase bg-orange-500/10 text-orange-600 dark:text-orange-400 border border-orange-500/20">
                Operations
              </span>
            </div>
            <p className="text-[11px] sm:text-xs text-slate-500 dark:text-zinc-400 font-medium hidden sm:block">
              Field & Production Operations Portal
            </p>
          </div>
        </div>

        {/* Action Controls: Theme Toggle, GAS Connection & User Badge */}
        <div className="flex items-center space-x-2 sm:space-x-3">
          {/* Light/Dark Mode Theme Toggle (Mounted in Navbar Header) */}
          <ThemeToggle />

          {/* GAS Connection Indicator Button */}
          <button
            onClick={() => setShowUrlModal(true)}
            className={`hidden md:flex items-center space-x-2 px-3 py-1.5 rounded-xl text-xs font-semibold border transition-colors ${
              currentGasUrl 
          ...
          {/* User Profile Badge */}
          {user && (
            <div className="flex items-center space-x-2 sm:space-x-3 pl-2 border-l border-slate-200 dark:border-zinc-800">
              <img
                src={user.photo || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=300&q=80"}
                alt={user.full_name}
                className="w-9 h-9 rounded-full object-cover border border-slate-300 dark:border-zinc-700 shadow-sm"
              />
              ...
              <button
                onClick={logout}
                className="p-2 rounded-xl text-slate-400 dark:text-zinc-400 hover:text-rose-600 hover:bg-rose-500/10 transition-colors"
                title="Sign Out"
              >
            ...
            <form onSubmit={handleSaveGasUrl} className="space-y-4">
              ...
              <div className="flex items-center justify-end space-x-3 pt-2">
                <button
                  ...
                  className="px-4 py-2.5 rounded-xl text-xs font-bold text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-zinc-800 hover:bg-slate-200 dark:hover:bg-zinc-700"
                >
                  Use Local Engine
                </button>
                <button
                  type="submit"
                  className="px-6 py-2.5 rounded-xl text-xs font-bold text-white bg-orange-500 hover:bg-orange-600 shadow-md shadow-orange-500/20 transition-all cursor-pointer"
                >
                  Save & Connect GAS
                </button>
              </div>
